  ...
  async function fetchBookings() {
    try {
      ...
      if (user && user.username)
        bookings = bookings.filter((bookings) => String(bookings.booker) === String(user.username));
      bookings.sort(
        (left, right) =>
          new Date(left.start_date || left.startDate || 0) -
          new Date(right.start_date || right.startDate || 0),
      );
    } catch (error) {
  ...
  onMount(async () => {
    try {
      ...
      try {
        ...
        logger.warn(
          "socket setup in myBookings failed",
          error && error.message ? error.message : error,
        );
      }
    } catch (error) {
      logger.error(
        "Error fetching bookings or resources",
        error && error.message ? error.message : error,
      );
    } finally {
      loading = false;
      ...
      <tbody>
        {#each bookings as booking}
          <tr class="align-top border-b">
            ...
            <td class="py-2">
              {formatDateOnly(booking.start_date || booking.startDate)} — {formatDateOnly(
                booking.end_date || booking.endDate || booking.start_date || booking.startDate,
              )}
            </td>
            <td class="py-2">{booking.comment || ""}</td>
            <td class="py-2">
              {#if booking.image}
                <a href={booking.image} target="_blank" rel="noopener" class="text-sm text-blue-600"
                  >View</a
                >
              {:else if resourceMap[String(booking.resource_id) || String(booking.resourceId)] && resourceMap[String(booking.resource_id) || String(booking.resourceId)].image}
                <a
                  href={resourceMap[String(booking.resource_id) || String(booking.resourceId)]
                    .image}
                  target="_blank"
                  rel="noopener"
