...
router.get("/api/users/username", isLoggedIn, async (req, res) => {
  try {
    const result = await db.query("SELECT username, email, role FROM users WHERE id = $1", [
      req.user.id,
    ]);
  ...
  return new Promise((resolve) => {
    req.session.regenerate((error) => {
      if (error) {
        ...
        return resolve(res.status(500).json({ message: "Internal server error" }));
      }
      req.session.user = {
        id: user.id,
...
router.delete("/api/users/:id", isLoggedIn, async (req, res) => {
  ...
  if (!idParam || !/^[0-9]+$/.test(String(idParam)))
    return res.status(400).json({
      message: "Invalid user id",
    });

  const requesterId = req.user && req.user.id ? String(req.user.id) : null;
  if (requesterId !== String(idParam) && req.user && req.user.role !== "admin")
  ...
  try {
    ...
    if (conflict && conflict.rowCount && conflict.rowCount > 0) {
      return res.status(409).json({
        message: "Cannot delete account while active bookings exist for your resources",
      });
    }
    ...
    await db.query(
      "DELETE FROM bookings WHERE resource_id IN (SELECT id FROM resources WHERE owner = $1)",
      [username],
    );
    await db.query(
      "DELETE FROM availabilities WHERE resource_id IN (SELECT id FROM resources WHERE owner = $1)",
      [username],
    );
  ...
  try {
        ...
        store.all((error, sessions) => {
          ...
          if (Array.isArray(sessions)) {
            for (const item of sessions) {
              const sessionId =
                item && (item.session_id || item.id || item.key)
                  ? item.session_id || item.id || item.key
                  : null;
              const session =
                item && (item.session || item.data) ? item.session || item.data : item;
    ...
    logger.debug(
      "Could not delete sessions for user",
  ...
  try {
    ...
    logger.debug(
      "Error while deleting resource images",
      error && error.message ? error.message : error,
    );
  }
  ...
  return res.status(200).json({
    message: "User account and related resources deleted successfully",
  });
});
  ...
  try {
    ...
    if (userData && userData.username) {
      ...
      const resourcesResult = await db.query(
        "SELECT id, name, type, owner, image FROM resources WHERE owner = $1 ORDER BY id",
        [username],
      );
