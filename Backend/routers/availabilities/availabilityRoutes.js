...
router.post("/api/availabilities", authLimiter, isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    try {
      if (global.io)
        global.io.to(`resource:${resourceId}`).emit("availability:changed", {
          resourceId,
          startDate,
          endDate,
          id: insertId,
...
router.delete("/api/availabilities/:id", isLoggedIn, async (req, res) => {
  try {
    ...
    const resourceOwner =
      resourceResult.rowCount && resourceResult.rows[0] ? resourceResult.rows[0].owner : null;
    ...
    try {
      if (global.io)
        global.io
          .to(`resource:${available.resource_id}`)
          .emit("availability:changed", { resourceId: available.resource_id });
    } catch (error) {
      logger.warn(
        "Failed to emit availability:changed on delete",
        error && error.message ? error.message : error,
      );
    }
