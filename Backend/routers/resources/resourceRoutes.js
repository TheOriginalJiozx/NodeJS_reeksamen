...
router.get("/api/resources", isLoggedIn, async (req, res) => {
  try {
    const result = await db.query(
      "SELECT id, name, type, owner, image FROM resources WHERE owner != $1 ORDER BY name",
      [req.user.username],
    );
...
router.post("/api/resources", isLoggedIn, async (req, res) => {
  try {
    ...
    if (typeof type === "number" || /^\d+$/.test(String(type))) {
      ...
      if (!typeResult.rowCount || !typeResult.rows[0])
        return res.status(400).json({ message: "Invalid type id" });
      typeName = typeResult.rows[0].name;
    }

    const result = await db.query("INSERT INTO resources (name, type, owner) VALUES ($1, $2, $3)", [
      name,
      typeName,
      owner,
    ]);
    ...
    if (insertId && imageUrl) {
      try {
        ...
        logger.debug(
          "Could not save image URL to resources table (column may be missing)",
    ...
    try {
      if (global.io)
        global.io.emit("resource:created", {
          id: insertId,
          name,
          type: typeName,
          owner,
      ...
      logger.warn(
        "Failed to emit resource:created",
        error && error.message ? error.message : error,
      );
    }
...
router.get("/api/resources/:id/availability", isLoggedIn, async (req, res) => {
  try {
    ...
    const datesBetween = (startString, endString) => {
      ...
      for (
        let current = new Date(startDateObject);
    ...
    for (const availableRows of availableResult.rows) {
      ...
      for (const dates of datesBetween(availableRows.startDate, availableRows.endDate))
        availableSet.add(dates);
    }

    for (const bookingRows of bookingsResult.rows) {
      if (!bookingRows.startDate || !bookingRows.endDate) continue;
      for (const dates of datesBetween(bookingRows.startDate, bookingRows.endDate))
        availableSet.delete(dates);
    }
...
router.post("/api/resources/:id/availability", isLoggedIn, async (req, res) => {
  try {
    ...
    if (!ownerCheck.rowCount || ownerCheck.rows.length === 0)
    ...
    try {
      if (global.io)
        global.io.to(`resource:${id}`).emit("availability:changed", {
          resourceId: id,
          startDate,
          endDate,
          id: insertId,
...
router.delete("/api/resources/:id", isLoggedIn, async (req, res) => {
  try {
    ...
    try {
      ...
      if (imagesRes.rowCount && imagesRes.rows[0] && imagesRes.rows[0].image)
        resourceImage = imagesRes.rows[0].image;
    } catch (error) {
      logger.debug(
        "Could not fetch resource image",
        error && error.message ? error.message : error,
      );
    }
    ...
    try {
      ...
      logger.debug(
        "Could not delete bookings for resource",
        error && error.message ? error.message : error,
      );
    }
    try {
      ...
      logger.debug(
        "Could not delete availabilities for resource",
        error && error.message ? error.message : error,
      );
    }
    ...
    if (resourceImage && String(resourceImage).includes("/uploads/")) {
      try {
        ...
        fs.unlink(uploadPath, (error) => {
          if (error)
            logger.debug(
              "Failed to delete resource image file",
              error && error.message ? error.message : error,
            );
        });
      } catch (error) {
        logger.debug(
          "Error deleting resource image file",
          error && error.message ? error.message : error,
        );
      }
    }
    ...
    try {
      ...
      logger.warn(
        "Failed to emit resource:deleted",
        error && error.message ? error.message : error,
      );
    }
