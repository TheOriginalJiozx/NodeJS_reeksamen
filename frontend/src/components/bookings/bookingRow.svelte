...
<td class="py-2">
  {#if booking.image}
    {#each getImages(booking.image).slice(0, 3) as image, index}
      <button type="button" on:click={() => openPreview(booking.image, resourceMap[String(booking.resourceId)] && resourceMap[String(booking.resourceId)].name, index)} class="p-0 rounded overflow-hidden mr-1 cursor-pointer hover:opacity-80 transition-opacity" aria-label={"Open image preview for " + ((resourceMap[String(booking.resourceId)] && resourceMap[String(booking.resourceId)].name) || "resource")}>
  ...
  {:else if resourceMap[String(booking.resourceId)] && resourceMap[String(booking.resourceId)].image}
    {#each getImages(resourceMap[String(booking.resourceId)].image).slice(0, 3) as image, index}
      <button type="button" on:click={() => openPreview(resourceMap[String(booking.resourceId)].image, resourceMap[String(booking.resourceId)].name, index)} class="p-0 rounded overflow-hidden mr-1 cursor-pointer hover:opacity-80 transition-opacity" aria-label={"Open image preview for " + ((resourceMap[String(booking.resourceId)] && resourceMap[String(booking.resourceId)].name) || "resource")}>
        <img src={image} alt="" class="h-8 w-12 object-cover rounded border" aria-hidden="true" />
      </button>
    {/each}
